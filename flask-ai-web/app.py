...
def fetch_history(limit: int = 50, action: str | None = None, q: str | None = None):
    conn = get_db()
    cur = conn.cursor()
...
def history():
    ...
    rows = fetch_history(limit=50, action=action, q=q)
    return render_template("history.html", rows=rows, action=action, q=q)
...
def history_delete(history_id: int):
    ...
    # ✅ 삭제 후에는 목록으로 돌아가기 (필터/검색 유지)
    return redirect(url_for("history", action=action, q=q))
